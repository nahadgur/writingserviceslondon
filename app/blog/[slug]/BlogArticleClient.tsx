    ...
    case 'image':
      return (
        <figure key={index} className="my-7">
          ...
          {block.alt && <figcaption className="body-sm text-center mt-2">{block.alt}</figcaption>}
        </figure>
      );
    case 'cta':
      return (
        <div key={index} className="inline-cta">
          ...
          <p className="body-sm text-white/50 mb-5">Vetted will writing professionals in London.</p>
          <button onClick={onOpenModal} className="btn-primary">Find my specialist</button>
        </div>
      );
        ...
        <div key={index} className="my-7">
          ...
          <div className="space-y-2">
            {(block.articles ?? []).map((rel, i) => (
              <Link key={i} href={`/blog/${rel.slug}/`} className="flex items-center gap-3 card-parchment p-3 rounded-md group">
                {rel.image && (
                ...
                <span className="font-display text-sm text-stone group-hover:text-brand transition-colors">{rel.title}</span>
              </Link>
            ))}
          </div>
        </div>
      ...
      <main>
        ...
        <section className="hero-dark" style={{ minHeight: 220 }}>
          ...
          <div className="relative z-10 container-width py-10 md:py-12 w-full">
            ...
            <h1 className="font-display text-3xl md:text-4xl lg:text-5xl italic text-white max-w-2xl leading-tight">
              {article.title}
            </h1>
          </div>
        </section>
        ...
        <div className="container-width py-10 md:py-14">
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_240px] gap-10 lg:gap-12">
            ...
            {/* Article */}
            <article>
              <Link href="/blog/" className="inline-flex items-center gap-2 body-sm text-dust hover:text-ink transition-colors mb-7">
                <ArrowLeft size={12} /> All articles
              </Link>

              <p className="article-lede mb-7">{article.excerpt}</p>

              {article.content.map((block, i) => (
                <div key={i}>
                  {i === secondH2 && (
                    <div className="pull-quote mb-7">
            ...
            {/* Sidebar — appears after article on mobile */}
            <aside>
                    ...
                    <div className="space-y-3">
                      {relatedArticles.map(rel => (
                        <div key={rel.slug}>
                          ...
                          <Link href={`/blog/${rel.slug}/`} className="font-display text-sm text-stone hover:text-brand transition-colors">
                            {rel.title}
                          </Link>
                        </div>
                      ))}
