...
import { AREA_HUBS, type AreaHub } from '@/data/locations';
...
function buildFaqs(hubName: string, subAreaNames: string[]) {
  return [
    { question: `How quickly can I be matched with a will writer covering ${hubName}?`, answer: `Most clients are matched within 24 hours. For urgent situations we can often arrange same-day introductions. We cover ${hubName} and surrounding areas including ${subAreaNames.slice(0, 4).join(', ')}.` },
    { question: `Is the matching service free for ${hubName} clients?`, answer: `Completely free. You only pay the specialist directly for their work. Our service is funded by the professionals in our network — no charge to clients at any stage.` },
    { question: `Do will writers offer home visits in ${hubName}?`, answer: `Yes. Most specialists in our network offer home visits across ${hubName} and surrounding areas. For elderly clients or those with mobility issues this is standard practice.` },
    { question: `What estate planning services are available in ${hubName}?`, answer: `Our network covering ${hubName} includes specialists in single wills, mirror wills, both types of Lasting Power of Attorney, protective property trusts, discretionary trusts, estate planning reviews, and probate support.` },
  ];
}

export function CityPageClient({ hub }: Props) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-[1fr_300px] gap-8 lg:gap-10 items-end">
              <div>
                ...
                <div className="flex items-center gap-2 mt-4 mb-4">
                  <span style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.4)' }}>{hub.postcode}</span>
                  <span style={{ color: 'rgba(255,255,255,0.2)', fontSize: 10 }}>&middot;</span>
                  <span style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: 'rgba(255,255,255,0.4)' }}>{hub.region} London</span>
                </div>
                <h1 style={{ ...serif('clamp(34px,5vw,56px)' as any, { color: '#fff', marginBottom: 14 }) }}>
                  Will writing in <span style={{ color: '#e8943a' }}>{hub.name}</span>
                </h1>
                <p style={{ fontFamily: 'var(--font-inter), sans-serif', fontSize: 14, fontWeight: 300, color: 'rgba(255,255,255,0.6)', maxWidth: 440, lineHeight: 1.75, marginBottom: 22 }}>
                  Free matching with vetted will writers and estate planning specialists covering {hub.name} and all surrounding areas. Within 24 hours.
                </p>
                <button onClick={() => setModal(true)} className="btn-primary lg:hidden">Find my specialist</button>
              </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_250px] gap-10 lg:gap-14">
            <div>
              ...
              <section className="mb-12">
                <h2 style={serif('clamp(20px,2.5vw,28px)' as any, { marginBottom: 10 })}>Areas we cover around {hub.name}</h2>
                <p className="body-md mb-5">Our will writers serve clients across {hub.name} and all surrounding neighbourhoods — including {hub.subAreas.slice(0, 3).map(s => s.name).join(', ')} and beyond.</p>
              ...
              {/* Services */}
              <section className="mb-12">
                <h2 style={serif('clamp(20px,2.5vw,26px)' as any, { marginBottom: 16 })}>Services available in {hub.name}</h2>
                  ...
                  {services.map(s => (
                    <Link key={s.id} href={`/services/${s.slug}/`} className="card group flex gap-3 p-4 items-start">
                      ...
                      <div style={{ minWidth: 0 }}>
                        <h3 style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: 15, color: 'var(--ink)', marginBottom: 3, transition: 'color 0.12s', lineHeight: 1.2 }} className="group-hover:text-brand-500">
                          {s.title} in {hub.name}
                        </h3>
              ...
              {nearbyHubs.length > 0 && (
                <section className="mb-12">
                  <h2 style={serif('clamp(20px,2.5vw,26px)' as any, { marginBottom: 12 })}>Nearby areas we cover</h2>
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                    {nearbyHubs.map(h => (
                      <Link key={h.slug} href={`/location/${h.slug}/`} className="card p-3 group" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <span style={{ width: 4, height: 4, borderRadius: '50%', background: 'var(--brand)', flexShrink: 0 }} />
                        <span style={{ fontFamily: 'var(--font-cormorant), serif', fontSize: 15, color: 'var(--ink)', transition: 'color 0.12s' }} className="group-hover:text-brand-500">{h.name}</span>
            ...
            <aside>
              <div className="lg:sticky" style={{ top: 28 }}>
                <div className="sidebar-box">
                  ...
                  <p className="body-sm mb-4">Free match with vetted will writers covering {hub.name} and surrounding areas.</p>
                  <button onClick={() => setModal(true)} className="btn-primary w-full justify-center">Get matched free</button>
                ...
                <div style={{ background: 'var(--ink)', borderRadius: 8, padding: '20px 18px' }}>
                  ...
                  <button onClick={() => setModal(true)} style={{ width: '100%', background: '#fff', color: 'var(--ink)', fontFamily: 'var(--font-inter), sans-serif', fontSize: 12, fontWeight: 500, padding: '10px', borderRadius: 4, border: 'none', cursor: 'pointer' }}>
                    Get free quotes
                  </button>
                </div>
              </div>
