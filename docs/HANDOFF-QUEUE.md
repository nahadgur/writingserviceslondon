...
## 2026-06-16 00:00 — wwsl-will-writer (writingserviceslondon)
- Sites: writingserviceslondon: new draft spoke "What a Surviving Spouse Inherits Without a Will" (what-surviving-spouse-inherits-no-will, hub intestacy-rules-uk, draft:true, dates 2026-06-16). Distinct intent vs the two existing intestacy spokes (avoid-intestacy-london = how to actively avoid intestacy; stepchildren-inheritance-rights-no-will = stepchildren). This one = the SURVIVING SPOUSE's entitlement under intestacy: the £322,000 statutory legacy, the spouse-vs-children split above it, the "spouse takes all where no issue" rule (deaths on/after 1 Oct 2014), a London worked example (£900k estate), the second-death/blended-family trap, and the spouse's right to appropriate the home. ~1,200 words incl FAQ, answer-first excerpt, 6 H2 sections + 5 distinct FAQs, UK English, no em dashes, matching framing ("we are a matching service and do not draft wills"). Did NOT target the hub head term "intestacy rules".
- Internal links: STRUCTURAL only (this site's blog 'p' renderer does NOT parse markdown — see fleet note below). Hub up-link renders from the `hub` field ("Part of our ... guide" -> /guides/intestacy-rules-uk/); intestacy tool link renders from toolByHub (/tools/#intestacy); pillar link added by registering the slug in BlogPostClient.tsx serviceMap -> /services/single-will/. No cross-hub or draft-sibling links. Externals (one per domain, attributed): GOV.UK (who-inherits-without-a-will) + Society of Will Writers.
- YMYL accuracy: statutory legacy stated as £322,000 since 26 July 2023 (verified via web search this run; macfarlanes/HughJames/GOV.UK corroborate). FLAG: data/guides.ts intestacy hub legalNote (section "married-with-children") wrongly states £322,000 applied "with effect from 6 February 2020" — that date was the £270,000 figure; £322,000 began 26 July 2023. Left untouched (read-only allowlist); recommend Claude Code fix the hub legalNote.
- Staged files: data/blog.ts (+ new spoke) and app/blog/[slug]/BlogPostClient.tsx (+1 serviceMap line) — COMMITTED in c8fcc5f. docs/SILO-PLAN.md section 8 tracker update (31 toward target: 20 live + 11 parked drafts; intestacy now 2 drafts) — on disk, NOT committed (see git state).
- Live flips this run: none (writer only).
- tsc: pass (npx tsc --noEmit clean; check-links.mjs: writingserviceslondon 42 slugs, 0 broken).
- Commit prompt / git state: content commit c8fcc5f IS on main (branch ahead 1 of origin/main). push FAILED (no GitHub credentials in sandbox) — please push. NOTE: safe-commit.sh's `git add -A` swept the prior run's uncommitted docs/SILO-PLAN.md + docs/HANDOFF-QUEUE.md edits into c8fcc5f (HANDOFF-QUEUE is not meant to be committed, but those were pre-existing working-tree changes). My follow-up tracker commit could NOT advance the ref: pre-existing stale .git/index.lock is unremovable from the sandbox ("Operation not permitted"). Windows-side to drain: delete stale .git/index.lock (+ any .git/*.lock / .git/objects/**/tmp_obj_*), then `git add docs/SILO-PLAN.md && git commit -m "content(intestacy): tracker update for surviving-spouse spoke" && git push`. Do NOT commit docs/HANDOFF-QUEUE.md or .next-dev.log.
- Renderer note (still open, flag for Claude Code): app/blog/[slug]/BlogPostClient.tsx renderBlock 'p'/'list' output block.text as PLAIN TEXT (no markdown parser), so inline [anchor](/url) body links render as literal "[text](url)" on live spokes. This spoke deliberately uses structural links instead (hub field + serviceMap + toolByHub), all real <Link>s. The how-much-does-probate-cost-london and avoid-intestacy-london drafts/live posts contain inline markdown links that will show literal brackets when live — recommend adding a small inline-link parser to the 'p'/'list' renderer (gate via /code-review, YMYL) and backfilling.
- Status: PENDING PUSH
